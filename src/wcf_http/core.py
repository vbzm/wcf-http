...
class Http(FastAPI):
    ...
    def __init__(self, wcf: Wcf, cb: str, **extra: Any) -> None:
        ...
        self.add_api_route("/forward_msg", self.forward_msg, methods=["POST"], summary="转发消息")
        # self.add_api_route("/revoke_msg", self.revoke_msg, methods=["POST"], summary="撤回消息")
        ...
        self.add_api_route("/attachment", self.download_attachment, methods=["POST"],
                           summary="（废弃）下载图片、文件和视频")
        ...
        self.add_api_route("/callback", self.get_callback, methods=["GET"], summary="Get the callback URL",
                           tags=["Callback"])
        self.add_api_route("/callback", self.set_callback, methods=["POST"], summary="Set the callback URL",
                           tags=["Callback"])
    ...
    def forward_msg(self,
                  id: int = Body("xxxxxx", description="转发消息ID"),
                  receiver: str = Body("filehelper", description="roomid 或者 wxid")) -> dict:
        ...
        Returns:
            int: 1 为成功，其他失败
        """
        ret = self.wcf.forward_msg(id, receiver)
        return {"status": ret, "message": "成功" if ret == 1 else "失败"}

    def revoke_msg(self,
                   id: int = Body(123456, description="要撤回的消息 id")) -> dict:
        """撤回消息
    ...
    def add_chatroom_members(self,
                             ...
                             wxids: str = Body("wxid_xxxxxxxxxxxxx",
                                               description="要加到群里的 wxid，多个用逗号分隔")) -> dict:
        """添加群成员
    ...
    def del_chatroom_members(self,
                             ...
                             wxids: str = Body("wxid_xxxxxxxxxxxxx",
                                               description="要删除的 wxid，多个用逗号分隔")) -> dict:
        """删除群成员
